...
export default function VanessaAI() {
  ...
  useEffect(() => {
    ...
    setMessages([
      { role: "assistant", content: isEnglish ? "Hello, I am Olivia AI." : "Hola, soy Olivia AI." },
  ...
  const send = async (event?: FormEvent, preset?: string) => {
    ...
    const metadata = { pageUrl: location.href, pageTitle: document.title, pageContent: document.body.innerText.replace(/\s+/g, " ").slice(0, 5000), dataConsent: true, privacyVersion: "hdm-olivia-2026-07-01", bookingDraft: contact.current, ...contact.current };
    try {
  ...
  return <div className="fixed bottom-5 right-5 z-[9998] font-sans text-[#2b211d]">
    {open && <section className="mb-3 flex h-[590px] max-h-[calc(100vh-100px)] w-[390px] max-w-[calc(100vw-28px)] flex-col overflow-hidden rounded-2xl border border-[#d7a928] bg-[#fef7e7] shadow-2xl">
      <header className="flex items-center justify-between bg-[#5d3b2d] px-5 py-4 text-white"><div><strong className="block text-xl text-[#f2cf68]">Olivia AI</strong><small className="font-bold text-red-300">Offline</small></div><button onClick={() => setOpen(false)} className="text-3xl">×</button></header>
      ...
      {!OFFLINE && <div className="flex gap-2 overflow-x-auto border-t border-[#eadcb8] p-2">{actions.map(([label, message]) => <button key={label} onClick={() => send(undefined, message)} className="shrink-0 rounded-full border border-[#d7a928] bg-white px-3 py-2 text-xs font-semibold">{label}</button>)}</div>}
      {!OFFLINE && !accepted && <button onClick={() => setPrivacy(true)} className="bg-white px-4 py-2 text-left text-xs underline">{isEnglish ? "Read and accept the Privacy Notice" : "Leer y aceptar el Aviso de Privacidad"}</button>}
      <form onSubmit={send} className="flex gap-2 bg-[#5d3b2d] p-3"><input value={input} onChange={(e) => setInput(e.target.value)} disabled={OFFLINE || !accepted || loading} placeholder={OFFLINE ? "Offline" : isEnglish ? "Write your message…" : "Escriba su mensaje…"} className="min-w-0 flex-1 rounded-xl bg-white px-4 py-3"/><button disabled={OFFLINE || !accepted || loading} className="rounded-xl bg-[#d7a928] px-4 font-bold disabled:opacity-50">{isEnglish ? "Send" : "Enviar"}</button></form>
    </section>}
    <button onClick={() => { setOpen(true); if (!OFFLINE && !accepted) setPrivacy(true); }} className="ml-auto flex items-center gap-2 rounded-full border border-[#d7a928] bg-[#5d3b2d] px-4 py-3 text-white shadow-xl"><span className="grid h-8 w-8 place-items-center rounded-full bg-[#d7a928] font-bold text-[#5d3b2d]">O</span>{OFFLINE ? "Offline" : isEnglish ? "Need help?" : "¿Necesita ayuda?"}</button>
    {!OFFLINE && privacy && <div className="fixed inset-0 z-[9999] grid place-items-center bg-black/70 p-5"><div className="max-h-[90vh] w-full max-w-xl overflow-auto rounded-2xl bg-white p-7"><h2 className="mb-4 text-2xl font-bold text-[#5d3b2d]">{isEnglish ? "Olivia AI Privacy Notice" : "Aviso de Privacidad de Olivia AI"}</h2><p className="mb-4 leading-relaxed">{isEnglish ? "Home Design Marques will process the name, email, phone, messages and visited page you share with Olivia AI to answer your request, prepare follow-up and contact you electronically." : "Home Design Marques tratará el nombre, correo, teléfono, mensajes y página visitada que comparta con Olivia AI para atender su solicitud, preparar el seguimiento y contactarle por medios electrónicos."}</p><a className="text-[#5d3b2d] underline" href={isEnglish ? "/en/privacy" : "/aviso-privacidad"} target="_blank">{isEnglish ? "Read the full Privacy Notice" : "Consultar el Aviso de Privacidad completo"}</a><label className="my-5 flex gap-2"><input type="checkbox" checked={checked} onChange={(e) => setChecked(e.target.checked)}/>{isEnglish ? "I have read and accept the Privacy Notice." : "He leído y acepto el Aviso de Privacidad."}</label><div className="flex justify-end gap-3"><button onClick={() => setPrivacy(false)} className="rounded-lg bg-gray-200 px-4 py-2">{isEnglish ? "Cancel" : "Cancelar"}</button><button disabled={!checked} onClick={() => { localStorage.setItem(`vanessaConsent:${CLIENT}`, "accepted"); setAccepted(true); setPrivacy(false); }} className="rounded-lg bg-[#d7a928] px-4 py-2 font-bold disabled:opacity-50">{isEnglish ? "Accept" : "Aceptar"}</button></div></div></div>}
  </div>;
}
