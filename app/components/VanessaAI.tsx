...
export default function VanessaAI() {
  ...
  useEffect(() => {
    if (OFFLINE) {
      setMessages([
        ...
        { role: "assistant", content: "Add more credit" },
      ]);
      return;
    }
    setMessages([
  ...
  return <div className="fixed bottom-5 right-5 z-[9998] font-sans text-[#2b211d]">
    {open && <section className="mb-3 flex h-[590px] max-h-[calc(100vh-100px)] w-[390px] max-w-[calc(100vw-28px)] flex-col overflow-hidden rounded-2xl border border-[#d7a928] bg-[#fef7e7] shadow-2xl">
      <header className="flex items-center justify-between bg-[#5d3b2d] px-5 py-4 text-white"><div><strong className="block text-xl text-[#f2cf68]">Vanessa AI</strong><small className="font-bold text-red-300">Offline · Add more credit</small></div><button onClick={() => setOpen(false)} className="text-3xl">×</button></header>
